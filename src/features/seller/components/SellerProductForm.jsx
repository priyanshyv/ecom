...
function SellerProductForm() {
  ...
  useEffect(() => {
    ...
    } else if (!params.id && user?.brand) {
      // For new products, set the brand to the seller's brand
      setValue('brand', user.brand);
    }
  }, [selectedProduct, params.id, setValue, user]);
  ...
  return (
    <form
      ...
      onSubmit={handleSubmit((data) => {
        ...
        // For sellers, always set the brand to their own brand
        if (user?.brand) {
      ...
      <div className="space-y-12 bg-white p-12">
        <div className="border-b border-gray-900/10 pb-12">
          <h2 className="text-base font-semibold leading-7 text-gray-900">
            {params.id ? 'Edit' : 'Add'} Product ({user?.brand})
          </h2>
          ...
          <div className="mt-10 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
            ...
            {/* For sellers, brand is fixed and not editable */}
            {user?.brand && (
